...
import { blog } from "@/app/api/keystatic/Collection";
...
export default async function Blog(props: {
  ...
  const currentId = post.title;
  const currentIndex = blog.findIndex((p) => p.entry.title=== currentId);
  const previousPost = blog[currentIndex - 1] ?? null;
  const nextPost = blog[currentIndex + 1] ?? null;

  const otherPosts = blog.filter((p) => p.entry.title !== currentId);
      ...
      <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
        ...
        <div className="relative py-16 overflow-hidden md:py-24">
          ...
          <div className="container relative z-10 px-4 mx-auto">
            <div className="flex flex-col items-center max-w-5xl gap-8 mx-auto md:flex-row md:gap-12">
  ...
  prose-abbr:border-b prose-abbr:border-dotted prose-abbr:border-gray-400 prose-abbr:cursor-help"
        ...
        <Carosello
          data={otherPosts.map(({ slug, entry }) => ({
            id: new Date(entry.publishDate).getTime(),
            title: entry.title,
