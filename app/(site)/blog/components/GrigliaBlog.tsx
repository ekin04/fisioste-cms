...
import { blog } from "@api/keystatic/Collection";
interface GrigliaBlogProps {}
export const GrigliaBlog: React.FC<GrigliaBlogProps> = () => {
  return (
    <>
      <div className="container pb-20 md:pt-10 m-auto grid md:grid-cols-2 xl:grid-cols-3 gap-16">
        {blog.map(({entry, slug}) => (
          <Link key={entry.title} href={`/blog/${slug}`}>
